...
public class PanelNuevaPartida extends JPanel implements ActionListener{
	...
	PanelNuevaPartida(){
		...
		titulo.setBounds((int) Math.round(Pantalla.width*0.035), (int) Math.round(Pantalla.height*0.1), Math.round((Pantalla.width*750)/1920), Math.round((Pantalla.height*100)/1080));
		titulo.setVisible(true);
		titulo.setFont(new FontUIResource("Calibri Bold", Font.PLAIN, Math.round((Pantalla.width*100)/1920)));
		titulo.setForeground(Color.white);
		
		// Creamos el subtitulo
		dificultad = new JLabel("DIFICULTAD");
		dificultad.setBounds((int) Math.round(Pantalla.width*0.04), (int) Math.round(Pantalla.height*0.3), Math.round((Pantalla.width*500)/1920), Math.round((Pantalla.height*100)/1080));
		dificultad.setVisible(true);
		dificultad.setFont(new FontUIResource("Calibri", Font.PLAIN, Math.round((Pantalla.width*60)/1920)));
		dificultad.setForeground(Color.white);
		
		// Creamos el boton de tutorial
		tutorial = new Boton("TUTORIAL", Pantalla.width*0.04, Pantalla.height*0.40, Math.round((Pantalla.width*225)/1920), Math.round((Pantalla.height*50)/1080));
		...
		tutorial.setFont(new FontUIResource ("Calibri" ,Font.PLAIN, Math.round((Pantalla.width*40)/1920)));
		
		// Creamos el texto para tutorial
		tTutorial = new TextArea("Practica cuanto quieras, no podrás perder", Pantalla.width*0.3, Pantalla.height*0.4, Math.round((Pantalla.width*400)/1920), Math.round((Pantalla.height*150)/1080));
		
		// Creamos el boton para facil
		facil = new Boton("FÁCIL", Pantalla.width*0.04, Pantalla.height*0.47, Math.round((Pantalla.width*225)/1920), Math.round((Pantalla.height*50)/1080));
		...
		facil.setFont(new FontUIResource ("Calibri" ,Font.PLAIN, Math.round((Pantalla.width*40)/1920)));
		
		// Creamos el texto para facil
		tFacil = new TextArea("Modo sencillo para aquellos \nque recién han empezado a jugar", Pantalla.width*0.3, Pantalla.height*0.4, 400, 150);
		
		// Creamos el boton para dificil
		dificil = new Boton("DIFÍCIL", Pantalla.width*0.04, Pantalla.height*0.54, Math.round((Pantalla.width*225)/1920), Math.round((Pantalla.height*50)/1080));
		...
		dificil.setFont(new FontUIResource ("Calibri" ,Font.PLAIN, Math.round((Pantalla.width*40)/1920)));
		
		// Creamos el texto para dificil
		tDificil = new TextArea("Modo dificil para quien sabe jugar", Pantalla.width*0.3, Pantalla.height*0.4, 400, 150);
		
		// Creamos el boton para personalizada
		custom = new Boton("PERSONALIZADA", Pantalla.width*0.04, Pantalla.height*0.61, Math.round((Pantalla.width*350)/1920), Math.round((Pantalla.height*50)/1080));
		...
		custom.setFont(new FontUIResource ("Calibri" ,Font.PLAIN, Math.round((Pantalla.width*40)/1920)));
		
		// Creamos el texto para personalizada
		tCustom = new TextArea("Elige los parámetros de tu partida \npara crear partidas únicas", Pantalla.width*0.3, Pantalla.height*0.4, Math.round((Pantalla.width*400)/1920), Math.round((Pantalla.height*150)/1080));
		
		// Creamos el boton para atras
		atras = new Boton("ATRÁS", Pantalla.width*0.035, Pantalla.height*0.8, Math.round((Pantalla.width*200)/1920), Math.round((Pantalla.height*50)/1080));
		atras.addActionListener(this);
		atras.MouseListener();
		atras.setFont(new FontUIResource ("Calibri" ,Font.PLAIN, Math.round((Pantalla.width*40)/1920)));
		
		// Creamos el boton para comenzar
		comenzar = new Boton("COMENZAR", Pantalla.width*0.425, Pantalla.height*0.8, Math.round((Pantalla.width*275)/1920), Math.round((Pantalla.height*50)/1080));
		comenzar.addActionListener(this);
		comenzar.MouseListener();
		comenzar.setVisible(false);
		comenzar.setFont(new FontUIResource ("Calibri" ,Font.PLAIN, Math.round((Pantalla.width*40)/1920)));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource()==comenzar){
			...
			if(c) {marco.add(new PanelPartidaPersonalizada()); marco.setVisible(true);}
			else {
				parametros();
				
				if(t) {
					marco.add(new PanelJuego());  PanelJuego.leer();}
				else if(f) {marco.add(new PanelJuego());}
				else if(d) {marco.add(new PanelJuego());}
			}
